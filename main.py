...
async def add_watermark_by_file(position: str = Query("centre", enum=["centre", "bottom_right","bottom_left","bottom"]),width_percentage: Optional[float] = Query("0.2"),insert_image: UploadFile=File(...),logo_image: UploadFile=File(...)):
    ...
    logo = Image.open(BytesIO(contents2)).convert("RGBA")
...
async def add_watermark_by_URL(image_url:str,logo_url:str,position: str = Query("centre", enum=["centre", "bottom_right","bottom_left","bottom"]),width_percentage: Optional[float] = Query("0.2")):
    ...
    response = requests.get(logo_url.lower())
